...
write `../../../packages/convex/.env.local`, generate types, push the current `convex/` code, and
...
`npx convex dev` writes the correct variable to `../../../packages/convex/.env.local` automatically.
...
3. `../../../packages/convex/.env.local` contains a `CONVEX_DEPLOYMENT` value and the framework's
...
- [ ] `../../../packages/convex/.env.local` has the deployment URL
